...
;1. Save the old rsp
        ...
        ;2. Count the number of the args
        mov rsi, PARAM(1); rsi - point to the first var in the list
	mov rdi, rsi
	mov rcx, 0
.L_bin_apply_args_counter:
	cmp rdi, sob_nil
	je .L_bin_apply_end_calc_loop
	assert_pair(rdi)
	mov rdi, SOB_PAIR_CDR(rdi)
	inc rcx
	jmp .L_bin_apply_args_counter
.L_bin_apply_end_calc_loop:
        
        ;3. Calculate the space to allocate on the stack
        lea r11, [8*(rcx - 3)]
        sub rsp, r11
        ...
        ;7. Push all args in argv[1] to the stack
        lea r9, [rsp + 8 * 3]
